...
export default function Interview() {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        const response = await fetch(`${API_BASE}/api/questions?type=${interviewType}`);
    ...
    if (currentStep < questions.length - 1) {
      ...
      try {
        const response = await fetch(`${API_BASE}/api/interview/submit`, {
          ...
          body: JSON.stringify({ type: interviewType, answers: newAnswers }),
        });
        const data = await response.json();
        if (response.ok) {
    ...
    <div className="container" style={{ paddingTop: '8rem', paddingBottom: '4rem' }}>
      <div className="grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2.5rem', alignItems: 'start' }}>
        <div style={{ position: 'sticky', top: '8rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          <div className="glass-card"><ProgressBar current={currentStep + 1} total={questions.length} /></div>
